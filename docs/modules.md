...
- `RsvpForm` y `RsvpFormVersion`: formulario versionado con ciclo de aprobación
  ...
  snapshot JSON validado de la versión. Crear un nuevo borrador conserva activa
...
- `RsvpSubmissionGuest` aporta `ResponseGuestId` estable y
  `RsvpSubmissionAnswer` conserva etiqueta, tipo, opciones, destinatario y
  sensibilidad como snapshots históricos.
...
- `RsvpService`: configuración, formulario, dashboard, excepciones, catálogos y
  recordatorios.
- `RsvpQuestionDefinitionParser`: deserialización estricta, catálogo backend y
  validación de publicación para tipos, opciones, reglas y árboles de
  visibilidad.
- `RsvpQuestionEngine`: resolución de alcance, evaluación controlada de
  visibilidad, obligatoriedad, normalización y errores estructurados.
- `RsvpSubmissionCoordinator`: envío público y administrativo dentro de una
  ...
  transporte y auditoría. Valida contra la versión exacta presentada antes de
  agregar filas y calcula el fingerprint con respuestas normalizadas.
- `RsvpSensitiveDataService`: lectura separada y auditada de datos sensibles,
  incluidas las respuestas de preguntas marcadas.
...
configuración, editor versionado y captura manual. El editor consume el
catálogo backend, limita reglas y referencias, detecta ciclos y simula
visibilidad. El wizard recalcula preguntas por grupo e invitado, limpia
respuestas ocultas y ubica errores estructurados sin persistir datos sensibles
en el navegador. La API conserva la decisión final.

El portal autenticado usa rutas propias bajo
...
- Las respuestas históricas se interpretan con su `RsvpFormVersionId` y
  snapshots; una publicación nueva no las evalúa de nuevo.
