...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| AUT-001 | Autenticación | `/auth/login` | `LoginPage` | Anónimo | — | 1 botón, 2 enlaces, 3 campos, 1 formulario | Iniciar sesión; crear espacio | Autenticar y volver al destino seguro | `POST /api/auth/login`, `GET /api/auth/me` | Cuenta activa | Sesión en memoria y contexto cargado | 400, 401, 429, red | Sí | Frontend/E2E mock/API integración | Parcial | Automatizado; API real cubierta en integración | — | Revisado automatizado |
| AUT-002 | Registro | `/auth/register` | `RegisterPage` | Anónimo | — | 1 botón, 2 enlaces, 9 campos, 1 formulario | Crear organización | Crear cuenta, organización, persona, Owner y sesión | `POST /api/auth/register-planner` | Correo no registrado | Alta transaccional y dashboard | 400, 409, 429 | Sí | Integración/E2E mock | Parcial | Automatizado | — | Revisado automatizado |
| SES-001 | Sesiones | Sin pantalla propia | `AuthService`, interceptor y guards | Autenticado | Sesión activa | Logout y restauración implícita | Cerrar sesión | Rotar refresh, cerrar sesión actual o todas | `/api/auth/refresh`, `/logout`, `/logout-all` | Cookie válida o sesión activa | Renovación segura; revocación inmediata | 401, origen inválido, reutilización | Sí | Unidad/integración/E2E mock | Parcial | Automatizado; multi pestaña manual pendiente | — | Parcial |
...
| POR-003 | Colaboración de invitados | `/portal/events/:id/guest-experience` | `PortalGuestExperiencePage` | Roles cliente autorizados | Permisos portal de invitados | 20 botones, 1 enlace, 14 campos, 3 formularios | Grupo; invitado; CSV; revisión; aprobar; comentarios; links; marcar | Colaborar sin publicar/regenerar/revocar/exportar | `/client-portal/events/{id}/guest-experience*` | Acceso al evento | DTO privado de portal | 400, 401, 404, 409 | Sí | Integración/E2E mock | Alta backend | Automatizado principal | — | Parcial |
...
| POR-005 | Captura RSVP portal | `/portal/events/:id/rsvp/capture` | `PortalRsvpCapturePage` | Cliente autorizado | Captura portal; sensibles separados | 1 botón, 1 enlace, 6 campos, 1 formulario | Registrar respuesta | Captura manual idempotente | `GET .../rsvp/form`, `POST .../manual-capture` | Acceso y grupo válidos | Revisión `ClientPortal` | 400, 401, 404, 409 | Sí | Integración/E2E mock | Alta backend | Automatizado parcial | — | Parcial |
...
| POR-008 | Pagos portal | `/portal/payments` | `PortalPaymentsPage` | ClientPayer/rol autorizado | Acceso y relación | 2 botones, 6 campos, 1 formulario | Reportar pago; comprobante | Crear pago pendiente y cargar recibo | `/api/client-portal/payment-plans`, `/payments*` | Plan visible | Pago pendiente; no aprobación | 400, 401, 404, 413, 415 | Sí | Integración/E2E mock | Alta backend | Automatizado principal | — | Parcial |
